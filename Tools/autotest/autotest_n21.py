...
def test_prerequesites():
    ...
    print("=>  Creating log folder @ " + util.reltopdir('../buildlogs') )
...
parser.add_option("--timeout", default=1000000, type='int', help='maximum runtime in seconds')
parser.add_option("--incremental", action='store_true', default=False, help='incremental build')
parser.add_option("--mode", type='string', default='kitchen-sink', help='fly only')
...
try:
    if not run_tests(steps[opts.mode]):
        sys.exit(1)
