...
#        trusty64.vm.network "public_network"
    ...
    config.vm.define "trusty32" do |trusty32|
        ...
        trusty32.vm.hostname = "apm-dev"
#        trusty32.vm.network "public_network"
        trusty32.vm.synced_folder(".", "/home/vagrant/ardupilot" )
        trusty32.vm.synced_folder("../PX4Firmware", "/home/vagrant/PX4Firmware" )
        trusty32.vm.synced_folder("../PX4NuttX", "/home/vagrant/PX4NuttX" )
        trusty32.vm.synced_folder("../mavlink", "/home/vagrant/mavlink" )

        trusty32.vm.provision "shell", inline: "cp /home/vagrant/ardupilot/puppet/Puppetfile /tmp"
        trusty32.vm.provision "shell", inline: "cp -R /home/vagrant/ardupilot/puppet/modules /tmp"
        trusty32.vm.provision "shell", inline: "cp -R /home/vagrant/ardupilot/puppet/manifests /tmp"

        # Puppet, install pre-reqs libraries
        trusty32.vm.provision "puppet" do |vb_provision_init|
            vb_provision_init.temp_dir = "/tmp"
            vb_provision_init.options = [ '--modulepath=/tmp/modules' ]
            vb_provision_init.manifests_path = [ "vm", "/tmp/manifests" ]
            vb_provision_init.manifest_file = "init.pp"
		...
		# Install librarian-puppet
        trusty32.vm.provision "shell", inline: "gem install librarian-puppet"
        trusty32.vm.provision "shell", inline: "cd /tmp && librarian-puppet install --verbose"

        # Puppet, finish the provisioning
        trusty32.vm.provision "puppet" do |vb_provision|
            vb_provision.temp_dir = "/tmp"
            vb_provision.options = [ '--modulepath=/tmp/modules' ]
            vb_provision.manifests_path = [ "vm", "/tmp/manifests" ]
            vb_provision.manifest_file = "apm_dev.pp"
        ...
        # VBox Manage
        trusty32.vm.provider :virtualbox do |vb_config|
            # Allow symlinks, your machine should be administrator
            vb_config.customize ["setextradata", :id, "VBoxInternal2/SharedFoldersEnableSymlinksCreate/v-root", "1"]
